...
const { learnTransactions, categorizeTransactions } = require("./classifier")
...
router.get('/descriptions/classify',async (req,res) => {
    let description = req.query.text;
    if(!description) return res.json([])
    let wrapper = {description:description}
    let categorized = await categorizeTransactions([wrapper])
